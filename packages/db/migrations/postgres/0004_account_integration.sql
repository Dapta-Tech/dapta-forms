...
--                         browser. `meta` holds display-only, non-secret hints
--                         (token last4, connected account label/hub id).
CREATE TABLE IF NOT EXISTS account_integration (
  ...
  CONSTRAINT account_integration_account_provider_uq UNIQUE (account_id, provider)
);
