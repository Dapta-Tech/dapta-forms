...
- 0b8b150: `isFilterableChoiceStep(step)` tells which steps the submissions table filters
...
- 504224b: `summarizeSubmissions(steps, rows)` builds the submissions Summary: one entry
...
- bfb3d2c: Submissions table and CSV: the Name question shows the answer.
...
- e802b03: Export `stepLabel`, `formatAnswerValue` and `formatAnswerCell` from the answer summary.
