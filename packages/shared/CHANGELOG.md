...
- 4a38827: Forms can render the attribution badge at a smaller size, chosen in the Design panel. Existing forms are unchanged.
...
- 59f1bbf: Feedback round (V3): authored step order is now authoritative at runtime —
  ...
  defaults (EN + ES).
- cbcef9c: Open an uploaded file in the dashboard instead of only downloading it.
...
- 8009d2d: Groundwork for a "file" question type: object storage, presigned uploads, and
...
- 40fe6fe: The file question, end to end: build it, answer it, download it.
...
- acb8823: Folders and keyboard search on the forms list.
...
- 42ec808: A form can name its language and its button copy.
...
- be7d8d1: Long text questions can require a minimum number of characters and cap the maximum, with a live counter on the public form.
...
- c97e73e: An option's stored value follows its label, and every pointer follows the value.
...
- 4f3077a: Screens in the builder: several questions on one slides screen.

  The layout copy now says what slides do with screens, in the create-form
  dialog and in Design: "One screen at a time. Group questions to show several
  on one screen." (`forms.layoutSlidesDesc`, `admin.editor.layout.slidesHint`).
  A new `admin.editor.layout.screensIgnored` note sits under the layout picker on
  One page when the form has screens: they only apply to Slides, and they are
  kept, so switching back restores them. Both locales.
...
- 002d388: Optional spam protection: a human check before a form's final submit, verified by the API.
...
- 8aadbbc: `admin.submissions.filters`: the copy for filtering and sorting the
  submissions from the column headings (the funnel's name, the menus, the chips
  above the table, the no-match state and the Summary's bar links), in English
  and Spanish. `statusAll`, `statusCompleted` and `statusPartial` are gone with
  the status buttons they labelled: the Status column filters now.
- f2db0fa: i18n: `admin.submissions.summary.*` (EN + ES) for the Summary tab of
  Submissions: the Summary | Responses switch, the per-question cards (the
  multi-select note, the slider average, files uploaded, meetings booked, the
  empty question) and the per-question answer search (label, placeholder,
  matches, no matches, show more, anonymous respondent, and its two failures).
...
- 2f17c60: A workspace timezone: every date in the admin, the analytics day cuts and the CSV local columns read in one shared zone.
...
- 5eda202: Connect's tracking help text now says when each event actually fires.
...
- 1d39e94: A "Docs" item in the rail, in the reader's language.
...
- 31c6e69: An embedded form's redirect now leaves at the top level instead of inside the iframe.
...
- f689309: Add the `admin.publish.saveFirst` message (EN + ES): shown when Publish could not land the pending autosave, so nothing was published.
- e23e13a: Scheduler step: pick from the list, or paste the event's link.
...
- 1438b3d: Scheduler step: the event-type list loads on the first try, and looks like it is loading.
...
- 339e6d8: i18n: `admin.chrome.workspaces.searchClear` and `admin.account.workspaces.searchClear` (EN + ES), the label of the clear button in the workspace switcher search and the workspace cards search.
...
- 4c64373: i18n: labels for the submissions response panel and the full-screen sheet (EN + ES): `viewResponse`, `responseTitle`, `prevResponse`, `nextResponse`, `closeResponse`, `responsePosition`, `answersTitle`, `detailsTitle`, `noAnswer`, `colStarted`, `responseId`, `utmTitle`, `answeredCount`, `sheetOpen`, `sheetClose`, `colResponse`, `colActions` and `scoreValue` under `admin.submissions`.
- 5b33838: i18n: labels for the submissions table tools (EN + ES) under `admin.submissions`: `pageSize`, `selectResponse`, `selectPage`, `selectedCount`, `selectedCountOne`, `exportSelected`, `clearSelection`, `bulkDeleteTitle`, `bulkDeleteTitleOne`, `bulkDeleteBody`, `bulkDeleteFailed` and `resizeColumn`.
