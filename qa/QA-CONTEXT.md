...
- Seeded demo form: `/acme/f/lead-qualifier`. Imported pilot-style form: `/acme/f/pilot-lead-qualifier`.
...
- **Create your own form** per spec via the admin API so specs stay independent:
  `POST http://localhost:4400/v1/forms` with `{name, config}` (JSON). The response returns `{id, slug}`. Public URL: `/<accountCode from GET /v1/me>/f/<slug>`, the neutral link the builder hands out. The middle segment is never read, so the older specs that navigate with `/me/` still reach the form, and that is what keeps old links covered.
...
- **Editor tabs**: Build · Logic · Connect (the general menu), synced to `?tab=` (deep-linkable). Testids `editor-tab-<id>`. **Design is NOT a nav tab** — it is a Build sub-mode entered from the contextual toolbar's `editor-tab-design` button (same testid, new home; it toggles the design view open/closed and carries `aria-pressed` while open, and `editor-tab-build` KEEPS `aria-current="true"` while design is up). `?tab=design` still parses and deep-links. **The duplicate mobile tab bar is gone** — one tab row at every width, so `editor-tab-<id>-mobile` no longer exists. **The Results tab is gone too**: its points list is Logic → Scoring and its score ranges are Logic → Outcomes. `?tab=results` still parses (it resolves to Build) so old links don't dead-end, but there is no tab to click. Old `/admin/forms/:id/integrations` route redirects to `edit?tab=connect`.
...
- **Preview is an iframe** (`preview-iframe`) rendering the REAL public renderer at a true viewport, fed the in-memory draft by postMessage. Reach inside with `frameLocator('[data-testid="preview-iframe"]')`; `live-preview` now lives in that document and carries `data-preview-state="waiting"|"ready"`. Three presets: `preview-device-mobile` (390×844) · `preview-device-tablet` (768×1024) · `preview-device-desktop` (1280×800), read back from `preview-viewport`. Screen navigation exists again — `preview-prev` / `preview-next` step the frame and `preview-position` reads "Cover" / "Step N of M" (slides layout only; it walks `runtimeSteps` with empty answers, i.e. the default branch). The "preview only" note sits behind an info icon (`preview-inert`, tooltip carries the copy); on vertical layout the sentence stays visible in the toolbar's left zone instead of the nav.
- **Editor topbar is two rows, Typeform-shaped.** Row 1 (the general menu, whole-form scope): back · name · save status · centred Build/Logic/Connect tabs · the three share icons · Publish. Row 2 (`editor-toolbar`, the builder's own submenu) changes with the active section — on Build AND on the design sub-mode: `toolbar-add-question` (primary) · `editor-tab-design` · the `canvas-device-*` switch (Build only) · `toolbar-preview` (the ORIGINAL `pi-eye`, not ▷). On Logic/Connect `toolbar-preview` sits at the row's end. The spine's dashed "+ Add question" is BACK at the bottom of the question list (Typeform has both routes into the gallery) — do not assert a single add button.
...
- **Google Sheets "coming soon" card** (`integration-gsheets`): an INERT announcement card after the HubSpot card in BOTH the Connect tab's integrations editor and `/admin/integrations` — no toggle, nothing focusable, `aria-disabled`, never writes to `config.destinations`. Do not count it as a real integration or expect a destination for it.
...
- **Settings moved behind the profile button.** The rail is Home · Forms · Submissions · Analytics · Integrations only; there is NO Settings or Branding rail item and no loose sign-out / view-public icons in the footer. Bottom-left of the rail (and of the mobile drawer) is a profile button `profile-menu-trigger` (avatar + name + email; avatar only when the rail is collapsed) that opens `profile-menu`: an "Account settings" eyebrow, then `profile-account-settings` (Workspaces → `/admin/account/workspaces`), `profile-nav-brand-kit`, `profile-nav-notifications`, `profile-nav-public-page`, a separator, and `profile-sign-out` (submits the sign-out action). `/admin/account` is Account settings with a left sub-nav `account-nav` (`account-nav-workspaces` · `account-nav-brand-kit` · `account-nav-notifications` · `account-nav-public-page`) → `/admin/account/workspaces` (cards `workspace-cards` › `workspace-card[data-account-id]` with `workspace-open` / `workspace-manage`, header `workspace-new`, search `workspace-search`; detail `/admin/account/workspaces/:id` with tabs `workspace-tab-members` / `workspace-tab-invitations`, `members-table` › `member-row[data-member-id]`, `invitations-table`; `:id` is the identity service's workspace id when the account was projected from one (`workspace-card[data-workspace-id]`, the local account id otherwise; both are accepted and the local one redirects to the canonical), shown in the header as `workspace-id` with `workspace-id-copy`; for staff the cards search also lists the estate under `workspace-estate` with Open-only cards `workspace-card[data-estate]`), `/admin/account/brand-kit`, `/admin/account/notifications`, `/admin/account/public-page`. Old URLs `/admin/settings` → `/admin/account/workspaces` and `/admin/branding` → `/admin/account/brand-kit` redirect. `workspace-name-form`, `workspace-name-input`, `workspace-name`, `pending-invitations`, `workspace-switcher*` and `rail-toggle` keep their ids. **On the editor route the rail peeks.** It rests at 64px and widens to 240px while the pointer is over it or the keyboard focus is inside it, overlaying the canvas instead of pushing it (a 64px spacer holds the column, so `<main>`'s box never moves) and painting at `z-30`: above the question cards, below the portalled rail menus at 55. `rail-toggle` renders on that route again and toggles the peek, but there it writes NOTHING: `forms.nav.collapsed` is neither read nor written inside the editor, so the saved preference survives a visit untouched.
