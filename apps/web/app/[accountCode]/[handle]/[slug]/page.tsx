...
import { canonicalFor } from '@/lib/public-form-path';
...
export async function generateMetadata({
  ...
  // The one URL this form should be known by: the neutral segment and the
  // current slug. The middle segment is decorative (the lookup above is
  // `getPublicForm(accountCode, slug)`, so `/acme/anything-at-all/my-form`
  // serves the form), and because it is now a fixed word rather than whatever
  // was typed, naming it here is stable: every spelling points at the same
  // address instead of minting a "canonical" of its own.
  //
  // Emitted when the request is not already that URL: a retired slug (the
  // redirect beside it is client-side, so this is the only signal a crawler, a
  // link checker or a social unfurler gets), or a link that still names a
  // member, which keeps serving the form where it is. No query string on it: a
  // canonical naming `?utm_source=...` would ask crawlers to index a
  // campaign's copy of the page as the original.
  const { canonical } = canonicalFor({ accountCode, handle, slug }, form.slug);
 ...
 *
 * The target is always the neutral `/{accountCode}/f/{slug}`, whatever middle
 * segment the visitor arrived with: a redirect we issue must not hand a
 * member's name back, and it keeps visitor input out of the `Location` header
 * (a handle of `%0d%0a` arrives decoded, as a real CRLF, and once made Node
 * answer 500). `accountCode` is still forwarded as it arrived, encoded: a
 * retired ACCOUNT code resolves transparently today, with no redirect
 * anywhere, and quietly rewriting it here would be a second behaviour change
 * smuggled in beside this one.
 */
function redirectToCanonical(
  ...
  const { redirect } = canonicalFor(route, canonical);
  if (redirect) permanentRedirect(`${redirect}${searchSuffix(query)}`);
}
...
export default async function PublicFormPage({
  ...
  // tag is what speaks to everything else. A link that only names a member in
  // the middle segment is NOT moved: it serves the form where it is and leaves
  // the correction to that tag (see `canonicalFor`).
